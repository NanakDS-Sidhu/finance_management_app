...
const Navbar = () => {
  ...
  return (
    <nav className="flex dark:bg-slate-900 items-center relative justify-between bg-zinc-800 px-5 py-6 w-full h-16">
      <div>
        <a href="/">            <Image src="/Mediamodifier-Design.svg" width={100} height={100} alt="logo"></Image></a>
      </div>
      <ul id="drawer" role="menu" className="sm:gap-3 transition-left ease-[cubic-bezier(0.4, 0.0, 0.2, 1)] delay-150  sm:flex  flex flex-col cursor-pointer absolute min-h-screen -left-48 sm:static w-48 top-0 bg-white sm:shadow-none shadow-xl sm:bg-transparent sm:flex-row sm:w-auto sm:min-h-0 dark:bg-slate-900  ">
        ...
        <li className="font-medium p-3 hover:text-white sm:p-0 sm:hover:bg-transparent text-gray-600">
          <a href="/setting" className="dark:text-white">Setting</a>
        </li>
        <li className="font-medium p-3 hover:text-white sm:p-0 sm:hover:bg-transparent text-gray-600">
      ...
      <div className="flex gap-3 items-center text-3xl">
        ...
        {status === 'loading' ? <>     <div className=" border-t-teal-400 w-8 h-8 border-4 border-blue-200 rounded-full animate-spin"></div>
          <span className="ml-2 text-base">loading</span></> : status === 'authenticated' ? (
            <>
